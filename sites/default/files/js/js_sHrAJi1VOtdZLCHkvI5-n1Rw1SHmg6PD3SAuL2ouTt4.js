		...
		types = {
			...
			"types" : {
				...
				"usage" : {
					...
					"icon" : {
						"image" : "/sites/default/files/root.png"
					}
				},
				"actionpricecharge" : {
...
;
(function($) {
	$.fn.loadSelectForm = function(node,select_type){
		var nodeId = node.attr('id');
		var loadLink = '/product_catalog_ajax/load_select_form/' + nodeId + '/' + select_type;
		...
		$.ajax({
			...
			success : function(data) {
				...
				$('#tree_content_div').append('<input id="vbo-select-button" type="button" value="Select"/>');
				$('#vbo-select-button').bind('click',$.fn.selectItemHandler);
				
				$(".product_catalog_tree").unmask();
				/*
				$('.vbo-views-form  > form:first').attr('action','');
				$('#edit-select').hide();
				
				//console.log(output[2]);
				//console.log(parent_nid);
				*/
			}
		});
	};
	
	$.fn.selectItemHandler = function(){
		...
		if(inputItems.length == 0){
			...
			var formUrl = $('.vbo-views-form  > form:first').attr('action');
			var urlArray = formUrl.split('/');
			
			var parentNid = null;
			for(var i=0;i<urlArray.length;i++){				
				if(!$.isNaN(urlArray[i])){
					parentNid = urlArray[i];
					break;
				} 
			} 
			...
			$.ajax({
				url : selectItemUrl,
				type: "post",
		        data: {'parent_nid': parentNid, 'child_nid': selectedItemNid, 'rel_type': $('#'+parentNid).attr('rel')},
				success : function(data) {
					var output = $.parseJSON(data);
					//console.log('output');
					//console.log(output);
					...
					if(product_catalog_ajax_result.data.attr.rel !== 'vouchercardtype') {
						var deleteNode = $('#' + product_catalog_ajax_result.data.attr.id);
						$.jstree._focused().delete_node(deleteNode);
					}
					//console.log('product_catalog_ajax_result');
					//console.log(product_catalog_ajax_result);
					$.fn.addMultipleChildren(product_catalog_ajax_result);
					var message = '<div style="display:block;"><div class="messages status"><h2 class="element-invisible">Status message</h2>'
						+ product_catalog_ajax_result.data.data[0] + ' has been changed.</div></div>';
					$('#tree_content_div').prepend(message);
					$(".product_catalog_tree").unmask();
				}
			});
		...
		$.ajax({
			...
			success : function(data) {
				...
				case 'basicrate_domestic':
					$.fn.loadForm(parent_id,'defaulttreenode','treeNodeBasicRateDomestic');
					break;
				case 'basicrate_idd':
					$.fn.loadForm(parent_id,'defaulttreenode','treeNodeBasicRateIDD');
					break;
				}
				$(".product_catalog_tree").unmask();
	...
	$.fn.loadForm = function(id,content_type,rel_type){
        ...
        if(needToLoadDetail){
        	var detailLink = '/product_catalog_ajax/load_select_form/' + id + '/' + rel_type;
        	$(".product_catalog_tree").mask("Loading...");
    		$.ajax({
    			url : detailLink,
    			success : function(data) {
	...
	$.fn.addMultipleChildren = function(product_catalog_ajax_result){
		var parentNode;
		var returnData;
		var tree = $.jstree._reference(".product_catalog_tree");
		//console.log(product_catalog_ajax_result);
		if(product_catalog_ajax_result.data.attr.rel === 'vouchercardtype') {
			parentNode = $('#'+product_catalog_ajax_result.data.attr.id);
        	returnData = tree._parse_json(product_catalog_ajax_result.data.children);
		} else {
			parentNode = $('#'+product_catalog_ajax_result.data.attr.parent_nid);
	...
	$.fn.addNode = function(parentNode, childContentType) {
		$("#tree_node_op").val('add');
		//console.log(childContentType);
		var parentNodeId = parentNode.attr('id');        
        var addLink = '/product_catalog_ajax/nojs/add/' + childContentType + '/' + parentNodeId;
        $(".product_catalog_tree").mask("Adding...");
		$.ajax({
			url : addLink,
			success : function(data) {
	...
	$.fn.deleteNode = function(node) {
		...
		$.ajax({
			...
			success : function(data) {
				...
				$('#tree_content_div').replaceWith('<div id="tree_content_div"></div>');
				$(".product_catalog_tree").unmask();
			}
		});
        //history.go(0);
	...
	$.fn.addConditionOperator = function(parentNode, childContentType) {
		$(".product_catalog_tree").mask("Adding...");
		var parentNodeId = parentNode.attr('id');

		var addLink = '/product_catalog_ajax/nojs/add/'
				+ childContentType + '/' + parentNodeId;

		$.ajax({
			url : addLink,
			dataType : 'json',
			success : function(data) {
				$.fn.addTreeItemCallback(data.parent_id, data.child_id,data.title, data.node_type, data.weight);
				$(".product_catalog_tree").unmask();
			}
		});
	};
	
	$.fn.addTreeItemCallback = function(parentNid, childNid, title,contentType, weight) {
		$(".product_catalog_tree").mask("Adding...");
		$(".product_catalog_tree").jstree("create", $('#' + parentNid),"inside", {"data" : title,"attr" : {"id" : childNid,"node_type" : contentType,"rel" : contentType,"weight" : weight}}, function() {}, true);
		$(".product_catalog_tree").unmask();
	};
})(jQuery);
;
(function($) {
	Drupal.behaviors.ProductCatalogTreeBehavior = {
		attach : function(context, settings) {	
			...
			//bind dbclick event for loading form
			$(".product_catalog_tree").bind("dblclick.jstree", function (e, data) {
	            ...
	            $.fn.loadForm(id,content_type,rel_type);  
	        });
